...
- `examples/travel.idl`: single-trip travel; people not things, a term ending on the return date, cover sections that start and stop on different days, a baggage limit per traveller.
- `examples/pet.idl`: lifetime pet cover; an annual limit per condition eroded by claims, a waiting period, a co-payment that arrives with age.
...
- `examples/household.idl`: buildings and contents as optional sections of one policy, each rated and settled on its own terms, with a discount for taking both.
