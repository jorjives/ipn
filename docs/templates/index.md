---
title: Templates
nav_order: 5
has_children: true
has_toc: false
---

# Templates

Starting points, one per shape of product. Each is a small working product with comments
that say what to change, and each passes its own scenarios, so you begin from something
proven and keep it that way. Copy the file from
[`templates/`](https://github.com/jorjives/open-idl/tree/main/templates), rename the product,
and run `check` after every change.

| Template | For |
|---|---|
| [Annual product](annual-product.md) | A single-term personal-lines product with every block in place |
| [Several items on one policy](multi-item-product.md) | A collection of items rated with for each |
| [Fixed-term benefit](fixed-term-benefit.md) | A benefit over a term of years, no renewal, no adjustment |
| [Commercial claims-made](commercial-claims-made.md) | Turnover-rated, aggregate limit, retroactive date, short-rate cancellation, commission |
| [Rated from a table](rated-from-a-table.md) | A multi-dimensional rating table kept as a spreadsheet |
| [A second version](versioned-product.md) | A product that changes its questions, with upgrading |

Not sure which? [Annual product](annual-product.md) fits most personal lines; the
[examples](../examples/index.md) show each shape grown into a real product.
