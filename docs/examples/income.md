...
> ```sh
> python3 -m ideclare check examples/income.idl
> ```

The file: [`examples/income.idl`](https://github.com/jorjives/open-idl/blob/main/examples/income.idl).

```idl
# Short Term Income Protection: a benefit paid over time rather than a loss made
# good. The claim says how long the customer was off work; a month's benefit is
# paid at the end of each month after the deferred period, the term's limit is a
# number of months of benefit in total, and a claim that starts late in the year
# keeps paying into the next.
# Run it with:  python3 -m ideclare check examples/income.idl
...
inputs
  ...
  # The most the policy pays in a year: the chosen number of months of benefit.
...
cover Unemployment optional
  limit monthly_benefit * 6 per term
  waiting period 90 days

rating
  # A rate on the annual benefit.
  base 3% of monthly_benefit * 12
  factor "Age"
    age < 30: x 0.80
    age < 40: x 1.00
    age < 50: x 1.40
  ...
  factor "Smoker"
    ...
    otherwise: x 1.00
  factor "Deferred period"
    deferred_weeks >= 26: x 0.50
    deferred_weeks >= 13: x 0.70
    deferred_weeks >= 8: x 0.85
    otherwise: x 1.00
  factor "Benefit period"
    benefit_months > 12: x 1.40
    otherwise: x 1.00
  load 35% when Unemployment selected
  minimum 60
  tax IPT 12%
...
lifecycle
  ...
  renewal
    ...
    index age by 1
    decline when age >= 65 because "Cover ends at 65"

claims
  claim Incapacity
    asks
      weeks_off_work: integer
      cause: choice of illness, injury, pregnancy, pre_existing
    requires gp_certificate
    # A month's benefit for each month off work after the deferred period, never more than the year's limit.
    pays monthly_benefit per month for ( weeks_off_work - deferred_weeks ) / 4 months after deferred_weeks weeks, up to limit
    decline when weeks_off_work <= deferred_weeks because "Absence did not outlast the deferred period"
    decline when cause is pre_existing because "Pre-existing conditions are excluded"
    decline when cause is pregnancy because "Normal pregnancy is not an incapacity"
  claim Unemployment
    asks
      weeks_unemployed: integer
    requires termination_letter
    pays monthly_benefit per month for ( weeks_unemployed - deferred_weeks ) / 4 months after deferred_weeks weeks, up to limit
    decline when weeks_unemployed <= deferred_weeks because "Unemployment did not outlast the deferred period"
...
scenario "A long absence is capped at the months of benefit left in the year"
  ...
  # 13 months' worth would be 19,500; 13,500 is left
  expect payout 13500.00
...
scenario "A claim late in the year keeps paying into the next"
  ...
  expect benefit paid 1500.00 by 2026-12-31
  expect benefit paid 4500.00 by 2027-02-28
  when renewed on 2027-01-01
  expect benefit paid 7500.00 by 2027-05-01
  # the new year's limit is untouched by a claim that arose in the old one
...
scenario "Excluded causes"
  ...
  when claim Incapacity for 0 on 2026-03-01 with gp_certificate, weeks_off_work 20, cause pregnancy
  expect claim declined "Normal pregnancy is not an incapacity"
  when claim Incapacity for 0 on 2026-03-01 with weeks_off_work 20, cause illness
...
scenario "Unemployment cover has an initial exclusion period"
  ...
  expect claim declined "Unemployment is within the 90 day waiting period"
  when claim Unemployment for 0 on 2026-05-01 with termination_letter, weeks_unemployed 20
  expect payout 4500.00
  when claim Unemployment for 0 on 2026-09-01 with termination_letter, weeks_unemployed 30
  # 8,250 would be due; 4,500 is left of the six months
  expect payout 4500.00
...
scenario "Cover ends at 65"
  ...
  when renewed on 2027-01-01
  when renewed on 2028-01-01
  when renewed on 2029-01-01
  when renewed on 2030-01-01
  when renewed on 2031-01-01
...
scenario "Two claims in a year load the renewal, within the cap"
  ...
  # 514.08 x 1.25 is exactly the 25% cap
  expect renewal premium 642.60
```
