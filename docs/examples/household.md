...
Household: buildings and contents sold as one policy. Each section is optional, rated on its own sum insured with its own excess and claims, and taking both earns a bundle discount. A customer must take at least one section.
...
# rated on its own sum insured with its own excess and claims, and taking both
# earns a bundle discount. A customer must take at least one section.
...
rating
  add "Buildings" 0.15% of rebuild_cost when Buildings selected
  add "Contents" 0.5% of contents_sum when Contents selected
